...
# **Deixando-um-locutor-de-fora, e depois treinando com tudo.**
#
# Com 1 ou 2 elocuções por pessoa não dá para dividir cada locutor: ou ele treina,
# ou ele mede. Pôr todos no treino faria o recall virar número otimista — o modelo
# teria visto toda voz que o teste contém.
#
# A saída é a validação cruzada por LOCUTOR: treina quatro vezes, cada uma sem uma
# pessoa, e mede o recall NA pessoa que ficou de fora. Isso estima generalização
# para voz nova sem desperdiçar nenhuma elocução — e só então o modelo final é
# treinado com tudo, que é o que vai para o APK.
tr_f = guido[:18]
te_f = guido[18:]
...
# ── validação cruzada por LOCUTOR ────────────────────────────────────────────
print("\nDEIXANDO-UM-LOCUTOR-DE-FORA (recall na voz que ficou de fora):")
todos = dict(outros); todos["guido"] = guido
Xneg, yneg = np.vstack([neg_leitura, neg_tr]), np.zeros(len(neg_leitura)+len(neg_tr), int)
for fora in todos:
    tr = [f for n, v in todos.items() if n != fora for f in (v[:18] if n == "guido" else v)]
    Xp_l = np.stack([vetor(v) for f in tr for v in aumentar(ler(f))])
    Xl = np.vstack([Xp_l, Xneg]); yl = np.concatenate([np.ones(len(Xp_l), int), yneg])
    e_l = StandardScaler().fit(Xl)
    c_l = LogisticRegression(max_iter=8000, C=0.1, class_weight="balanced").fit(e_l.transform(Xl), yl)
    alvo = todos[fora] if fora != "guido" else guido[18:]
    sp = c_l.predict_proba(e_l.transform(np.array([vetor(ler(f)) for f in alvo])))[:, 1]
    sn = c_l.predict_proba(e_l.transform(neg_te))[:, 1]
    print(f"  sem {fora:<6} → recall@0,5 {(sp>0.5).mean():>4.0%} ({len(alvo)} elocuções) · "
          f"escore min {sp.min():.3f} · FP {disparos(sn,0.5)/horas:>5.2f}/h")

# ── modelo final: TODO MUNDO no treino ───────────────────────────────────────
tr_todos = guido[:18] + [f for v in outros.values() for f in v]
Xp2 = np.stack([vetor(v) for f in tr_todos for v in aumentar(ler(f))])
X2 = np.vstack([Xp2, Xneg]); y2 = np.concatenate([np.ones(len(Xp2), int), yneg])
esc2 = StandardScaler().fit(X2)
clf2 = LogisticRegression(max_iter=8000, C=0.1, class_weight="balanced").fit(esc2.transform(X2), y2)
sn2 = clf2.predict_proba(esc2.transform(neg_te))[:, 1]
print(f"\nFINAL (todos os {len(tr_todos)} positivos no treino), no negativo retido:")
for lim in (0.5, 0.6, 0.7, 0.9):
    print(f"  limiar {lim}: {disparos(sn2,lim)/horas:.2f}/h · escore max no negativo {sn2.max():.3f}")

w = clf2.coef_[0]/esc2.scale_
b = float(clf2.intercept_[0] - (clf2.coef_[0]*esc2.mean_/esc2.scale_).sum())
np.concatenate([w.astype(np.float32), np.float32([b])]).tofile("cabeca_v4.f32")
print(f"\n  cabeca gravada: cabeca_v4.f32 ({(len(w)+1)*4} bytes)")
