...
- **BATIMENTO ALCANÇÁVEL** — ele não existia. O `minDistance` suprime a entrega (*"the potential
  location update will not occur"*, AOSP), então agente parado não recebia callback e a linha do
...
- **PORTA DE CORREÇÃO**: degradação relativa (teto fixo mataria o Standby), salto por
  **incerteza combinada** (distância crua mataria a rede) e **válvula de 3 recusas** — sem ela um
  salto verdadeiro congela o marcador, pior que sumir porque mente parado. E `ultimaPosicao()`
  decide idade antes de precisão.
- **Ferramentas:** `par_headless.mjs` · `sonda_de_politica.py` · dois agentes autenticados.

## O que está quebrado, e nós sabemos

1. **O detector NÃO tem chamador em `src/main` do app.** Pela régua do §6 isto é **escrito**, não
...
2. **O falso positivo não tem intervalo útil.** `0` em 1,8 min dá limite superior de **~99/h**
   com 95%; a meta é **0,5/h**, e isso exige da ordem de **6 h** de leitura.
...
9. **A porta de correção nunca viu GPS ruim de verdade** — testes sintéticos, e o emulador
   entrega 5 m sempre.
10. `errorStream` não coletado · `STOPPED` não terminal · câmera do DAT nunca pedida.

**Pendências:** três cláusulas da Fase 2 presas a dispositivo HFP · `security-crypto` alpha.

## O que vem a seguir

**A Fase 3 fechou inteira** — nove itens, o último o batimento com idade real.

1. **Ligar o detector de ativação** no app: sem chamador, e é o item mais antigo dos quebrados.
2. Fase 4 (conhecimento de domínio on-device) e Fase 5 (UX e ensaio).
