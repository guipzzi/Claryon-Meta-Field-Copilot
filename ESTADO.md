...
- Três telas compostas atrás de permissões e login: guarnição (PTT por toque + botão
  "Perguntar ao copiloto"), mapa MapLibre acompanhando o portador com rumo, e perfil.
- **PTT ponta a ponta:** 16 kHz consistente, `transmit`/`ack` deployadas, `transmissions`
  recebendo INSERT, fio do canal populado. Latência toque→1º quadro **31–48 ms** (meta 120).
  `AgrupadorDeQuadros` reduz 50 msg/s → ~17.
...
- **Rota de saída sustentada** (`RotaSustentada`): a rota SCO atravessa a rajada de fala em
  vez de subir e cair a cada emissão. Era o que fazia o earcon de P1 esperar uma remontagem
  de rota para soar.
...
- Posição: `public.consultar_posicao(indicativo)` tira o solicitante do JWT e
  `private.posicao_relativa` está revogada de `authenticated`. Garantia do servidor.
...
- Verificado por `javap`: o DAT 0.9.0 **não** expõe microfone e o MockDeviceKit **não**
  simula áudio nem alimenta a câmera emulada. Óculos e fone reais seguem sendo a única
  bancada honesta.
...
3. `WakeWordDetector` é interface sem implementação, e `PowerPolicy` declara
   `wakeWordAtiva = true`. O modo **Standby** é inalcançável. O `KeywordSpotter` do
   sherpa-onnx está no AAR e sem chamador.
...
6. Transcrição na origem (P1) ainda não existe: falta o acumulador, o Whisper no `finally`
   e o quarto evento no protocolo.
7. Duas filas de prioridade que não se enxergam foi **resolvido** para o corte, mas o TTS do
   ciclo de voz e o do rádio compartilham o dono único só na saída — não na síntese.
