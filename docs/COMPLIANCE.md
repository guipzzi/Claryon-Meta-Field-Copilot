...
| **Output por áudio** | Reprodução via `AudioTrack` (`USAGE_VOICE_COMMUNICATION`); TTS lacônico (≤7 palavras, com teste); earcons sintetizados; fila de prioridade | ✅ a fala sai **e** os earcons tocam — `SaidaUnica` (`app/src/main/.../audio/`) é o dono único da saída e importa `core-sound`. **10 earcons, distinguibilidade par a par com teste** (`DistinguibilidadeDosEarconsTest`), incluindo o ataque de 120 ms e ruído de viatura | `core-sound/…/EarconSynthesizer.kt`, `PrioritySoundQueue.kt` |
| **Privacidade e dados** | On-device no caminho crítico; **zero** reconhecimento facial; fala de terceiros não transcrita (rota HFP é pré-condição da captura); cofre `EncryptedFile` + Keystore com cadeia de hash — adulterar 1 byte aponta o segmento, e a **âncora de fim** pega truncamento | ⚠️ o cofre é instanciado e alimentado, mas **nada em `src/main` confere** o que ele grava, e o **rádio não entra nele** | `core-evidence/…/EncryptedEvidenceVault.kt`, `HashChain.kt`, `AncoraDeFim.kt` |
...
- ✅ Cofre cifrado + cadeia de custódia: `EncryptedFile` (AES-256-GCM) + Keystore, hash encadeado, manifesto parcial a cada segmento. **Adulterar 1 byte → aponta o segmento** (teste instrumentado). Instanciado em `CopilotoDoAgente:170` e alimentado por `Intent.IniciarGravacao`.
- ✅ **Truncamento fechado** (22/08): hash encadeado é cego a remoção no fim — apagar os últimos segmentos e as linhas `S` deixava uma cadeia perfeita e um veredito de integridade. Manifesto **v3** leva âncora de fim (HMAC-SHA256 do Keystore); a conferência fecha por falta. 19 testes instrumentados no aparelho + 12 em JVM; mutação derruba exatamente os 5 novos. `AncoraDeFim.kt`.
- ✅ **Caminho de conferência no produto** (22/08): `verificar()` e `Manifesto.ler()` tinham **zero chamadores em `src/main`** — o produto selava a âncora e conferia só em teste, e periciar exigia `adb`/root sobre o diretório privado. `EncryptedEvidenceVault.periciar()` fecha o buraco: **Perfil → Periciar a custódia** lista as gravações seladas com o veredito tipado de cada uma (`Integra`, `Truncada`, `SemAncoraDeFim`, `ExpurgadaPorPolitica`, `Quebrada`, `SegmentoNaoRegistrado`) e distingue gravação **em curso** de processo morto antes de fechar. 9 testes em JVM + 4 instrumentados; contra-teste derruba o build ao remover o chamador.
- ⚠️ **Sem exportação:** a tela **confere**, não extrai. Tirar segmentos e manifesto do aparelho continua exigindo `adb`, e a tela declara isso. E `Integra` **não** é "inforjável" — a chave da âncora é usável pelo próprio app (ver R8 do relatório de impacto); a ressalva está na tela, com teste que reprova se ela sumir.
- ❌ **O rádio (PTT) não entra no cofre.** Os dois únicos chamadores partem de `Intent.IniciarGravacao`; nada no caminho do PTT escreve evidência. De uma transmissão sobrevive a **transcrição** no servidor, não o áudio.
