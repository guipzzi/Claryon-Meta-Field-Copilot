...
- ❌ **Áudio, transcrição ou frame para serviço externo** no caminho crítico.
...
| **R4** | Vazamento de evidência no aparelho | `EncryptedFile` + Android Keystore, com cadeia de hash — adulterar um byte aponta o segmento | O manifesto de custódia **ainda não é assinado** com chave do Keystore. **Declarado no código** (`Manifesto.kt:26-32`) e pendente |
| **R5** | Envio de dado sensível a terceiro | Nenhuma chamada a serviço externo no caminho crítico. IA 100% local: os três modelos estão **dentro do APK** | Base veicular oficial é consulta a sistema do próprio Estado, com `procedencia` declarada em toda resposta |
...
1. O manifesto de custódia **não é assinado** — apenas encadeado por hash.
