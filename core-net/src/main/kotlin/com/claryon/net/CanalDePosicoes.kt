...
class CanalDePosicoes(
    ...
    private val espelho = LinkedHashMap<String, PosicaoDePar>()
     ...
     */
    suspend fun assinar(talkGroupId: String): Boolean {
        if (!publicador.publicando()) return false
        assinado = publicador.assinarPares(talkGroupId)
        return assinado
    }

    /** Fecha a assinatura e **descarta o espelho**. */
    suspend fun desassinar() {
        assinado = false
    ...
    fun marcadorDe(indicativo: String, minhaLat: Double, minhaLon: Double): MarcadorDePar? {
        ...
        val p = espelho.values.firstOrNull { normalizar(it.indicativo) == alvo }
            ?: espelho.values.firstOrNull { normalizar(it.indicativo).startsWith(alvo) }
            ?: return null
        return marcador(p, minhaLat, minhaLon, agora())
    }
